...
export default function App() {
  ...
  // Fetch real backend products if Spring Boot Gateway is running
  useEffect(() => {
    fetch('/api/products')
      .then(res => res.json())
      .then(data => {
      ...
      .catch(() => {
        // Standalone UI preview fallback mode
        setServiceStatus(true);
      });
  }, []);
  ...
  // Saga Orchestration Checkout Logic
  const executeSagaCheckout = async (itemsToBuy) => {
    ...
    try {
      ...
      // Fallback client simulation if backend is initializing
    }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <footer style={{
        ...
        <p>Saga Orchestrator Production E-Commerce Platform • Java 17 • Spring Boot 3 • ReactJS Vite • MySQL • Redis</p>
      </footer>
    </div>
  );
}
